...
class PgClient
  ...
  def get_all_parents
    execute_query('select id, url FROM urls WHERE parent_id IS NULL AND scraped = 0')
  end

  def insert_url(url)
    execute_query('INSERT INTO urls (url, scraped) VALUES ($1,0)', [url])
  end

  def update_scrape_url_status(url_id, status)
    execute_query('UPDATE urls SET scraped = $1 WHERE id = $2', [status, url_id])
  end

  def get_child_urls(parent_url_id)
    execute_query('SELECT * FROM urls where parent_id = $1 ORDER BY ID DESC LIMIT 1', [parent_url_id])
  end
